# ai-cooking-agent
AI Cooking Agent for College Students — recommends recipes based on budget, time, cuisine, and dietary constraints, using TF-IDF retrieval and a Hugging Face LLM.
...
- **Agent loop**: single-step retrieval is used by default, with an optional second retrieval round if needed.
...
The focus is on prompting, retrieval, and agent orchestration rather than model finetuning.
...
- agent with basic prompting
- agent with constraint-aware prompting

## 6. Discussion
Our results suggest that adding structured prompting improves both retrieval targeting and final recommendation quality.
When the agent fails, common causes include vague user constraints or missing coverage in the recipe collection.
Future improvements could include:
- expanding the recipe dataset
- better cost estimation features
- multi-step planning (e.g., weekly meal prep)

## 7. Conclusion
We built a lightweight AI cooking agent for college students using TF-IDF retrieval and an open-source LLM.
The project demonstrates a practical, reproducible pipeline for agentic information retrieval and grounded recommendation.
Although limited by dataset size, the system shows promising performance for simple real-world cooking queries.
